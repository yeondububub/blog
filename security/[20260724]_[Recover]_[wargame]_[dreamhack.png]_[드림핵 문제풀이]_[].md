...
단, 4바이트 길이의 키(`v6` 배열의 값)를 모른다는것이다. 하지만 원본 파일이 **PNG 이미지라는** 사실을 통해 키를 알아낼 수 있다.
...
키의 길이가 4바이트이므로, 암호화된 파일의 **첫 4바이트와** PNG 헤더의 첫 4바이트(`89 50 4E 47`)를 비교하면 역산하여 4바이트 키를 완전히 복구할 수 있다.
...
**C언어 (암호화):** `ptr` 변수는 1바이트 크기(`char`)이다. 1바이트가 담을 수 있는 숫자는 `0 ~ 255`뿐이다. 만약 250이라는 값에 19를 더해서 269가 되면, 한계치인 255를 넘어가서 **오버플로우(Overflow)가** 발생하고 값이 **13이** 된다.

**파이썬 (복호화):** 파이썬의 숫자(정수)는 크기 제한이 없다. 암호문이 13일 때 복호화를 위해 단순히 `13 - 19`를 하면 파이썬은 **-6이라는** 음수를 내뱉는다. 바이트에는 음수가 들어갈 수 없으므로 에러가 나거나 엉뚱한 값이 된다.
